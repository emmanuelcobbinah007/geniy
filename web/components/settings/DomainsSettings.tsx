...
export function DomainsSettings() {
  ...
  const verifyDomainMutation = useMutation({
    mutationFn: async (domainId: string) => {
        if (!token || !workspaceId) return
        return api.verifyDomain(workspaceId, domainId, token)
    },
    onSuccess: () => {
  ...
  const deleteDomainMutation = useMutation({
    mutationFn: async (domainId: string) => {
        if (!token || !workspaceId) return
        return api.deleteDomain(workspaceId, domainId, token)
    },
    onSuccess: () => {
                ...
                {domains.length === 0 ? (
                    ...
                    <div className="grid gap-4">
                        {domains.map((domain: Domain) => (
                            <div key={domain.id} className="border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 md:p-6 space-y-4 bg-zinc-50/50 dark:bg-zinc-900/30">
                                <div className="flex items-start justify-between">
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            <h3 className="font-semibold text-lg text-zinc-900 dark:text-white">{domain.domain}</h3>
                                            {domain.status === 'active' ? (
                                                <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border-0">Active</Badge>
                                            ) : (
                                                <Badge variant="outline" className="text-amber-600 border-amber-200 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-800">Pending Verification</Badge>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {domain.status !== 'active' && (
                                            <Button size="sm" variant="outline" onClick={() => verifyDomainMutation.mutate(domain.id)} disabled={verifyDomainMutation.isPending}>
                                                {verifyDomainMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4 mr-2" />}
                                                Verify
                                            </Button>
                                        )}
                                        <Button size="icon" variant="ghost" className="text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20" onClick={() => deleteDomainMutation.mutate(domain.id)} disabled={deleteDomainMutation.isPending}>
                                            {deleteDomainMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                                        </Button>
                                    </div>
                                </div>
                                    ...
                                    <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                            ...
                                            <div className="p-3 bg-zinc-50 dark:bg-zinc-900 rounded border border-zinc-100 dark:border-zinc-800">
                                                ...
                                                <div className="font-mono">{domain.domain.split('.')[0]}</div>
                                            </div>
                                            <div className="p-3 bg-zinc-50 dark:bg-zinc-900 rounded border border-zinc-100 dark:border-zinc-800">
                                                <div className="text-xs text-zinc-500 uppercase tracking-wider font-medium mb-1">Value</div>
                                                <div className="font-mono">cname.{rootDomain}</div>
                                            </div>
                                        </div>
                                    </div>
                                )}

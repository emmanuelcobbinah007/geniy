...
export const api = {
  ...
  async saveIntegrations(workspaceId: string, data: any, token: string) {
  ...
  // AI Integration
  async analyzeContext(contextText: string, token: string, workspaceId?: string, recommendations?: string[], missingDimensions?: string[]) {
    return this.post('/ai/analyze-context', { contextText, workspaceId, recommendations, missingDimensions }, token);
  },
  ...
  async deleteCompetitor(workspaceId: string, competitorName: string, token: string) {
